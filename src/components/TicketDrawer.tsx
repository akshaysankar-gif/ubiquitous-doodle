...
import { mockData, SENTIMENT_BANDS } from "@/lib/mockData";
import { Icon, Pill, Button } from "@/components/ui/PrototypeKit";
import { SentimentBar, SentimentChip, Bullet, Tag } from "@/components/ui/PrototypePrimitives";
...
export const TicketDrawer = ({ monthKey }: { monthKey?: string }) => {
  const { drawer, closeDrawer, month } = useSupport();
  
  const currentMonth = monthKey || month;
  
  if (!drawer.isOpen || !drawer.ticketId) return null;

  const payloadStr = drawer.ticketId;
  let payload: any = { type: 'unknown' };
  
  if (payloadStr.startsWith('cell-')) {
    const parts = payloadStr.split('-');
    payload = { type: 'cell', module: parts[1], intent: parts[2] };
  } else if (payloadStr.startsWith('code-')) {
    payload = { type: 'code', code: payloadStr.replace('code-', '') };
  } else if (payloadStr.startsWith('ticket-')) {
    const tId = payloadStr.replace('ticket-', '');
    const t = mockData.tickets.find(x => x.id === tId);
    payload = { type: 'ticket', ticket: t };
  } else if (payloadStr.startsWith('area-')) {
    const parts = payloadStr.replace('area-', '').split('::');
    payload = { type: 'area', module: parts[0], area: parts[1] };
  }

  let tickets = mockData.tickets.filter((t: any) => t.month === currentMonth && t.team === 'Technical Support' && t.status !== 'Archived');
  let title = 'Tickets', subtitle = '';

  if (payload.type === 'cell') {
    tickets = tickets.filter((t: any) => t.module === payload.module && t.intent === payload.intent);
    title = `${payload.module} · ${payload.intent}`;
    subtitle = `${tickets.length} tickets · avg frustration ${(tickets.reduce((a: any, t: any) => a + t.frustration, 0) / (tickets.length || 1)).toFixed(1)}/10`;
  } else if (payload.type === 'code' || payload.type === 'area') {
    const code = payload.code || (payload.module + ' · ' + payload.area);
    tickets = tickets.filter((t: any) => (t.module + ' · ' + t.area) === code);
    title = code;
    subtitle = `${tickets.length} tickets`;
  } else if (payload.type === 'ticket' && payload.ticket) {
  ...
  tickets.forEach((t: any) => {
    ...
    if (sent[t.sentiment] !== undefined) sent[t.sentiment]++;
    if (t.isAI) aiN++;
  });

  return (
    <>
      <div onClick={closeDrawer} style={{ position: 'fixed', inset: 0, background: 'rgba(17,17,17,0.32)', zIndex: 50 }} />
      <aside style={{
        ...
        <div className="scroll-area" style={{ flex: 1, overflow: 'auto' }}>
              ...
              <div style={{ marginTop: 6, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                {SENTIMENT_BANDS.map(b => (
                  <span key={b.key} style={{ fontSize: 10.5, color: 'var(--ss-fg-muted)' }}>
                    <span style={{ display: 'inline-block', width: 7, height: 7, borderRadius: 99, background: b.swatch, marginRight: 4 }} />{b.label}: <strong style={{ color: 'var(--ss-fg)' }}>{sent[b.key] || 0}</strong>
                  </span>
                ))}
          ...
          <div style={{ padding: '14px 22px', borderBottom: '1px solid var(--ss-neutral-100)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {Object.entries(subareas).sort((a: any, b: any) => b[1] - a[1]).slice(0, 5).map(([s, n]: any) => (
                <div key={s} style={{ display: 'grid', gridTemplateColumns: '1fr 40px', gap: 8, alignItems: 'center' }}>
                  <div>
                    ...
                    <Bullet value={n / Math.max(...Object.values(subareas) as number[])} color="var(--ss-primary-500)" height={4} />
                  </div>
          ...
          <div style={{ padding: '14px 22px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
              ...
              <Button size="sm" variant="outline" style={{ color: 'var(--ss-secondary-700)', borderColor: 'var(--ss-border-strong)' }}>
                <Icon name="ExternalLink" size={14} /> Open in Zendesk
              </Button>
            </div>
...
function summarize(tickets: any[], intents: any, subareas: any) {
  const n = tickets.length;
  if (!n) return 'No tickets matched this slice.';
  const top = Object.entries(intents).sort((a: any, b: any) => b[1] - a[1])[0] as any;
  const sub = Object.entries(subareas).sort((a: any, b: any) => b[1] - a[1])[0] as any;
