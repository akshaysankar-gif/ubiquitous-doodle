// Configurable bubble chart. Multiple encoding presets:
//   volume-frustration   X=volume,        Y=avg frustration, size=effort, color=product/family
//   volume-automation    X=volume,        Y=automation pot,  size=effort, color=module
//   growth-volume        X=MoM growth%,   Y=volume,          size=frustration, color=root-cause family
//
// Renders an SVG plot with axis labels, grid, optional bubble labels.
...
const BubbleChart = ({data, encoding='volume-frustration', height=420, onHover, onClick, highlightKey=null}) => {
  ...
  // Domain
  let xs = data.map(d=>d[E.x.key]);
  let ys = data.map(d=>d[E.y.key]);
  ...
  const sx = (v) => margin.l + ((v-xMin)/(xMax-xMin || 1))*innerW;
  const sy = (v) => margin.t + innerH - ((v-yMin)/(yMax-yMin || 1))*innerH;

  // size scale
  const sizes = data.map(d=>d[E.size.key]);
  ...
  // Sort by size desc so smaller bubbles render on top
  const sorted = [...data].sort((a,b)=>b[E.size.key]-a[E.size.key]);
    ...
    <div style={{display:'flex',gap:14, flexWrap:'wrap', alignItems:'center'}}>
      {entries.map(([k,c])=>(
        <span key={k} style={{display:'inline-flex',alignItems:'center',gap:6,fontSize:11.5,color:'var(--ss-fg)'}}>
          <span style={{width:10,height:10,borderRadius:99,background:c}}/>{k}
        </span>
      ))}
    </div>
  );
...
Object.assign(window, { BubbleChart, BubbleLegend, ENCODINGS });
